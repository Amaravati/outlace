...
#def predict(model, x):
##    W1, b1, W2, b2 = model['W1'], model['b1'], model['W2'], model['b2']
#    modelin=np.loadtxt('modelin.txt')
...
def main():
#    X, y = generate_data()
    X=np.loadtxt('/home/anvesha/test_rc_car/tests.out')
    y=np.loadtxt('/home/anvesha/test_rc_car/testa1.out')
    y=y.astype(int)
    print(y)
    no_epoch=5000
    model = build_model(X, y, 100,2000, print_loss=True)
    W1, b1, W2, b2 = model['W1'], model['b1'], model['W2'], model['b2']
    modelin=np.concatenate((W1,b1),axis=0)
    modelout=np.concatenate((W2,b2),axis=0)
...
#    visualize(X, y, model)
    out=calculate_loss(model, X, y)

    [out,prob]=predict(X)
    print(out)
    print(prob)
    error=np.not_equal(out,y)
    errorf=np.mean(error)
    print("error is:{}" .format(errorf))
    Xn=np.array([[4, 4, 3]])
    pr=predict(Xn)
    print(pr)
    return W1,b1,W2,b2,modelin,modelout


if __name__ == "__main__":
    [W1,b1,W2,b2,modelin,modelout]=main()
...
#    print("update value is {}" .format(y[0][action]))
#    print("Game #: %s" % (i,))
